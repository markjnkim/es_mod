...
We've made several big changes to our webpage over these last few lessons. We built a fairly complicated grid, adjusted the layout to be more flexible, and added media queries to ensure true mobile compatibility. While these were all super important aspects to take care of, the marketing team at Run Buddy is still itching to see the landing page stand out more visually. Hence the "CSS enhancements and optimizations" issue that we created earlier.
...
These changes bring more professionality to the landing page, which the marketing team hopes will lead to more sign-ups. At the same time, Run Buddy is expecting an improved user experience in the form of visually interactive elements. While HTML links and buttons are already technically interactive, their out-of-the-box styles aren't very enticing to modern-day web users.
...
3. Change the header to be "sticky" so it follows the page as it scrolls

4. Style the different states of our HTML elements to make them feel more interactive 
...
> **Important:** If you aren't able to switch branches due to uncommitted work, or you simply want to discard changes that don't need to be staged in Git, use the command `git checkout -- path/to/file` to reset individual files. For example, `git checkout -- index.html` would reset `index.html` to its last commit.

## Add Shadows and More

Our first code edit will be to add shadows to some of our HTML elements. This is something the design team requested to give the elements more depth. In CSS, shadows come in two flavors, text shadows and box shadows. They're represented by the following CSS declarations:
...
>
> You can experiment with RGB and CMYK values using an online color picker. These are so common that [Google provides a color picker directly in its search results.](https://www.google.com/search?q=color+picker&oq=color+picker) 
...
Here's another example, one we'll use in Run Buddy:
...
Add this `text-shadow` to the following CSS rules in your style sheet: 
...
Save your CSS file, refresh the browser, and enjoy your new shadows! There's one more shadow we need to add to the hero form itself, though. `text-shadow` wouldn't be appropriate here; it would add shadows to all of the text inside the form! Instead, let's use `box-shadow`. Add this declaration to your `.hero-form` rule:
...
> **Deep Dive:** Since browsers are built by different companies, it's no surprise that their underlying **engines** behave differently. Browser engines read the HTML code of a webpage and convert it into something the user can actually see and interact with. Safari's engine is called WebKit. Chrome was originally based on WebKit but has become its own thing now called Blink. Recently, Microsoft adopted Blink as the engine for the Edge browser. There's clearly a lot of sharing that happens across these engine projects, but discrepancies still exist, of course.
>
...
Now that the header is sticky, we can style it to look like part of the hero. Keep in mind that the header isn't really part of the hero. The header and hero need to be separate elements in order for the stickiness to work. If done correctly, however, the user won't notice.
...
But wait... when the input is in focus, there's an extra blue border around it, which looks terrible when paired with our rounded corners! This is the browser's default styling at play. We can't add a `border-radius` to this outline, but we can remove it altogether by including a second declaration:
...
What we can do, though, is make the default buttons invisible and use CSS to create and position our own buttons on top of them. This is a good compromise, because screen readers will ignore the CSS that will create the custom buttons while still picking up the real buttons underneath.
...
The `opacity` property works much like the alpha value in `rgba()`, where transparency is defined on a scale of 0.0 to 1.0. This turned all of the inputs invisible, but if you inspect the page with the Chrome DevTools, you can see that the elements are still there:
...
A higher `z-index` brings the element to the front while a lower `z-index` pushes it to the back. The value `9999` may seem like overkill in this case. `z-index: 1;` would still fix our header/form problem. We may end up needing to add `z-index` to other elements, though, and we want to ensure the header is always on top, so we'll go with `9999` to avoid future conflicts.
...
This lesson gave us a chance to experiment with some of the newer and cooler features of CSS, making our Run Buddy landing page truly stand out. This greatly improved the user experience and gives legitimacy to Run Buddy being a modern company.

Granted, that doesn't mean we should apply shadows and gradients to every element on the page. Too much "wow" can be noisy. Used sparingly, however, these visual tricks can add depth, highlight important information, or simply make the page feel more interactive and interesting.

In the next lesson, we'll look at another hot topic in the world of the web: animation. But first, let's revisit some of the highlights:
