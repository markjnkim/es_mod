...
>    3. The Dragstart Event
>    4. Define the Drop Zone
>    5. The Drop Event
>    6. The Dragleave Event
...
<!-- We can accomplish this by preventing the default behavior on just the task lists. We can identify the drop zone  with the `.task-list` class attribute. The `dropZoneDragHandler()` should now look like this:
```js
var dropZoneDragHandler = function(event) {
...
``` -->
<!-- Let's breakdown this conditional statement to understand how it is checking for our desired drop zones. The first part of the conditional, `event.target.closest(".task-list")`, is using the `closest()` method.  -->
...
``` 
<!-- 
If the `target` is outside of the task list elements, the conditional statement will evaluate to null and prevent dropping. This is precisely what we would like. 
